...
namespace MPGungeon.MPServer.Canvas
{
     public class CanvasImage
    {
        ...
        public CanvasImage(GameObject parent, Texture2D tex, Vector2 pos, Vector2 size, Rect subSprite)
        {
            ...
            Vector2 position = new Vector2((pos.x + ((size.x / subSprite.width) * subSprite.width) / 2f) / Screen.width, (Screen.height - (pos.y + ((size.y / subSprite.height) * subSprite.height) / 2f)) / Screen.height);
        ...
        public void SetPosition(Vector2 pos)
        {
            if (imageObj != null)
            {
                Vector2 position = new Vector2((pos.x + ((sz.x / sub.width) * sub.width) / 2f) / Screen.width, (Screen.height - (pos.y + ((sz.y / sub.height) * sub.height) / 2f)) / Screen.height);
